...
--[[ Required-level bands ]]--
--Same bands as GrimfallLevelBags: 1-9, 10-19, 20-29, ... 50-59, 60, 61+, no requirement.

Sort.LEVEL_LABELS = {
  'Level 1-9', 'Level 10-19', 'Level 20-29', 'Level 30-39',
  'Level 40-49', 'Level 50-59', 'Level 60', 'Above level 60',
  'No level required',
}

function Sort.LevelBand(minLevel)
  minLevel = minLevel or 0
  if minLevel <= 0 then return 9 end
  if minLevel < 10 then return 1 end
  if minLevel < 60 then return math.floor(minLevel / 10) + 1 end
  if minLevel == 60 then return 7 end
  return 8
end


--[[ Equipment type filter ]]--
--Armor subtype comes straight from GetItemInfo; "weapon 1H/2H" is derived
--from the equip slot, since that's not a single subtype in the game data.
...
Sort.TYPE_FILTERS = {
  ...
  {key = 'weapon1h', label = 'Weapon 1H'},
  {key = 'weapon2h', label = 'Weapon 2H'},
}

local ARMOR_SUBTYPE_KEY = {Cloth = 'cloth', Leather = 'leather', Mail = 'mail', Plate = 'plate'}
local WEAPON_1H_SLOTS = {
  INVTYPE_WEAPON = true, INVTYPE_WEAPONMAINHAND = true,
  INVTYPE_WEAPONOFFHAND = true, INVTYPE_HOLDABLE = true,
}
local WEAPON_2H_SLOTS = {
  INVTYPE_2HWEAPON = true, INVTYPE_RANGED = true, INVTYPE_RANGEDRIGHT = true,
}

--returns a Sort.TYPE_FILTERS key for the item, or nil if it matches none of them
function Sort.TypeFilterKey(link)
  if not link then return nil end
  local _, _, _, _, _, _, subType, _, equipLoc = GetItemInfo(link)
  if equipLoc and WEAPON_1H_SLOTS[equipLoc] then return 'weapon1h' end
  if equipLoc and WEAPON_2H_SLOTS[equipLoc] then return 'weapon2h' end
  if subType and ARMOR_SUBTYPE_KEY[subType] then return ARMOR_SUBTYPE_KEY[subType] end
  return nil
end
  ...
  for _, bag in itemFrame:GetVisibleBags() do
    local family = Bagnon.BagSlotInfo:GetBagType(itemFrame:GetPlayer(), bag)
      ...
      if link then
        ...
        item.minLevel = itemMinLevel or 0
        item.levelBand = Sort.LevelBand(itemMinLevel)
        item.name = itemName
      end
		end
	end
...
function Sort:GetOrder(spaces, family)
  ...
  local rule = (self.itemFrame:IsLevelGroupEnabled() and self.LevelRule) or self.Rule
  sort(order, rule)
  return order, slots
end
...
--used instead of Sort.Rule when the item frame's Level Group layout is on,
--so the "clean items" button physically matches what's shown on screen:
--lowest required-level band first, then required level, then name.
function Sort.LevelRule(a, b)
  if a.levelBand ~= b.levelBand then
    return a.levelBand < b.levelBand
  end
