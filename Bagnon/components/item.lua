...
function ItemSlot:Create()
	...
	--required-to-use level, shown in the corner opposite the stack count
	local level = item:CreateFontString(nil, 'OVERLAY', 'NumberFontNormalSmall')
	level:SetPoint('TOPLEFT', item, 'TOPLEFT', 2, -2)
...
--required-to-use level (blank for items with no level requirement)
function ItemSlot:SetLevel(link)
	local minLevel = link and select(5, GetItemInfo(link))
	if minLevel and minLevel > 0 then
		self.levelText:SetText(minLevel)
		self.levelText:Show()
	else
		self.levelText:Hide()
	end
