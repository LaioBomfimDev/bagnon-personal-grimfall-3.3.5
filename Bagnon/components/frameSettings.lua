...
--level group layout
function FrameSettings:SetLevelGroup(enable)
	local enable = enable and true or false

	if self:IsLevelGroupEnabled() ~= enable then
		self:GetDB():SetLevelGroup(enable)
		self:SendMessage('ITEM_FRAME_LEVEL_GROUP_UPDATE', self:IsLevelGroupEnabled())
	end
end

function FrameSettings:IsLevelGroupEnabled()
	return self:GetDB():IsLevelGroupEnabled()
end

--equipment type filter
function FrameSettings:SetTypeFilter(key)
	if self:GetTypeFilter() ~= key then
		self:GetDB():SetTypeFilter(key)
		self:SendMessage('ITEM_FRAME_TYPE_FILTER_UPDATE', self:GetTypeFilter())
	end
end

function FrameSettings:GetTypeFilter()
	return self:GetDB():GetTypeFilter()
end
