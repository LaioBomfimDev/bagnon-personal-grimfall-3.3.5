...
--level group layout (groups items by required-to-use level band)
function SavedFrameSettings:SetLevelGroup(enable)
	self:GetDB().levelGroup = enable
end

function SavedFrameSettings:IsLevelGroupEnabled()
	return self:GetDB().levelGroup
end

--equipment type filter (only used by the level-group layout for now)
function SavedFrameSettings:SetTypeFilter(key)
	self:GetDB().typeFilter = key
end

function SavedFrameSettings:GetTypeFilter()
	return self:GetDB().typeFilter
end
