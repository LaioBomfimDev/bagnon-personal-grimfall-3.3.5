...
function ItemFrame:ITEM_FRAME_LEVEL_GROUP_UPDATE(msg, frameID, enable)
	if self:GetFrameID() == frameID then
		self:RequestLayout()
	end
end

function ItemFrame:ITEM_FRAME_TYPE_FILTER_UPDATE(msg, frameID, key)
	if self:GetFrameID() == frameID then
		self:RequestLayout()
	end
end
...
function ItemFrame:UpdateEvents()
	...
	if self:IsVisible() then
		...
		self:RegisterMessage('ITEM_FRAME_LEVEL_GROUP_UPDATE')
		self:RegisterMessage('ITEM_FRAME_TYPE_FILTER_UPDATE')
...
local LEVEL_HEADER_HEIGHT = 16
local LEVEL_HEADER_GAP = 4
local LEVEL_GROUP_GAP = 8

--groups items by required-to-use level band (1-9, 10-19, ... 60, 61+, no requirement),
--with an unsorted "free slots" bucket last. Purely visual: does not move items
--in the real bags, unlike the sort button.
function ItemFrame:Layout_Level()
	self.needsLayout = nil
	self.levelHeaders = self.levelHeaders or {}

	local columns = self:NumColumns()
	local spacing = self:GetSpacing()
	local effItemSize = self.ITEM_SIZE + spacing
	local LevelBand, LABELS = Bagnon.Sorting.LevelBand, Bagnon.Sorting.LEVEL_LABELS
	local TypeFilterKey = Bagnon.Sorting.TypeFilterKey
	local typeFilter = self:GetTypeFilter()
	local FREE_BAND = #LABELS + 1

	local groups = {}
	for i = 1, FREE_BAND do groups[i] = {} end
	...
	for _, bag in self:GetVisibleBags() do
		for slot = 1, self:GetBagSize(bag) do
			...
			if itemSlot then
				...
				if link then
					if typeFilter and typeFilter ~= '' and TypeFilterKey(link) ~= typeFilter then
						itemSlot:Hide()
					else
						itemSlot:Show()
						local name, _, _, _, minLevel = GetItemInfo(link)
						local list = groups[LevelBand(minLevel)]
						list[#list + 1] = {itemSlot = itemSlot, minLevel = minLevel or 0, name = name or ''}
					end
				else
					itemSlot:Show()
					local list = groups[FREE_BAND]
					list[#list + 1] = {itemSlot = itemSlot, minLevel = 0, name = ''}
				end
			end
		end
	end
	...
	local y, maxWidth, headersUsed = 0, 0, 0

	for band = 1, FREE_BAND do
		local list = groups[band]
		if #list > 0 then
			headersUsed = headersUsed + 1
			local header = self:GetLevelHeader(headersUsed)
			header:ClearAllPoints()
			header:SetPoint('TOPLEFT', self, 'TOPLEFT', 0, -y)
			header:SetText((LABELS[band] or 'Free slots') .. '  (' .. #list .. ')')
			header:Show()
			y = y + LEVEL_HEADER_HEIGHT + LEVEL_HEADER_GAP

			for i, entry in ipairs(list) do
				local row = (i - 1) % columns
				local col = math.ceil(i / columns) - 1
				entry.itemSlot:ClearAllPoints()
				entry.itemSlot:SetPoint('TOPLEFT', self, 'TOPLEFT', effItemSize * row, -y - effItemSize * col)
			end

			local rows = math.ceil(#list / columns)
			maxWidth = math.max(maxWidth, effItemSize * math.min(columns, #list) - spacing)
			y = y + effItemSize * rows - spacing + LEVEL_GROUP_GAP
		end
	end

	for i = headersUsed + 1, #self.levelHeaders do
		self.levelHeaders[i]:Hide()
	end

	self:SetWidth(math.max(maxWidth, effItemSize * columns - spacing))
	self:SetHeight(math.max(1, y - LEVEL_GROUP_GAP))
end

function ItemFrame:GetLevelHeader(index)
	self.levelHeaders = self.levelHeaders or {}
	local header = self.levelHeaders[index]
	if not header then
		header = self:CreateFontString(nil, 'ARTWORK', 'GameFontNormal')
		header:SetHeight(LEVEL_HEADER_HEIGHT)
		header:SetJustifyH('LEFT')
		self.levelHeaders[index] = header
	end
	return header
end
...
function ItemFrame:IsLevelGroupEnabled()
	return self:GetSettings():IsLevelGroupEnabled()
end

function ItemFrame:GetTypeFilter()
	return self:GetSettings():GetTypeFilter()
end